...
\section{Experience}
  \resumeSubHeadingListStart
    \resumeSubheading
      ...
      \resumeItemListStart
        \resumeItem{Global Fixed Income - Data}
          {Infrastructure team that is in charge of both real-time and reference data for the entire department. Kafka, Airflow, Python, C++, gRPC, Data Pipeline, Redis}
          \resumeItemListStart
            \workItem{}
              {\textbf{Real-time tick pipeline} that handles 13 billion ticks a day}
            \workItem{}
              {Decreased in process performance time by 80\% using perf flamegraph.}
            \workItem{}
              {Decreased memory footprint of each worker in the cluster from 15GB RAM to 1 GB by optimizing based on sharded scope.}
            \workItem{}
              {\textbf{Reference data} scraping, parsing and management}
            \workItem{}
              {Increased read APIs fault tolerance by implementing retry mechanisms around database connectivity which reduced to 0 support tickets.}
          \resumeItemListEnd
        \resumeItemListEnd
    \resumeSubheading
      ...
      \resumeSubheading
        ...
        \resumeItemListStart
          \workItem{}
          {Implemented a synchronization tool between a desktop application and an external CRM system (workbooks.com)}
          \workItem{}
          {Improved the FAFSA (Federal Student Aid) questionnaire by fixing bugs and adding extra features.}
        \resumeItemListEnd
  \resumeSubHeadingListEnd
